...
describe("enrollFreeCourse", () => {
  ...
  // ── Permission: grant is always scoped to input.userId ─────────
  it("scopes the AccessGrant to input.userId", async () => {
    await enrollFreeCourse({
      userId: "user_alice",
      productSlug: "test-course-e2e",
    });
    const upsertCall = mockPrisma.accessGrant.upsert.mock.calls[0][0];
    expect(upsertCall.create.userId).toBe("user_alice");
  });

  // ── Idempotency: sourceId is deterministic from (userId, productId) ─
  it("generates a deterministic sourceId from userId and productId", async () => {
    await enrollFreeCourse({
      userId: "user_1",
      productSlug: "test-course-e2e",
    });
    const upsertCall = mockPrisma.accessGrant.upsert.mock.calls[0][0];
    expect(upsertCall.where.sourceType_sourceId_productId.sourceId).toBe(
      "free_enrollment:user_1:prod_123",
    );
  });
});
